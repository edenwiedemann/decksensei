...
export default function PromptRollbackButton({ gameId, activeVersion, candidates }: Props) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<PromptCandidate | null>(null);
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!loading) { setOpen(v); setSelected(null); setError(""); } }}>
        <DialogContent className="max-w-lg" aria-describedby="rollback-prompt-desc">
          <DialogHeader>
            <DialogTitle>Voltar versão de prompt</DialogTitle>
            <DialogDescription id="rollback-prompt-desc" className="text-sm text-muted-foreground">
              Versão ativa atual:{" "}
              ...
              {candidates.map((c) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    {c.activated_at && (
                      <p className="mt-0.5 text-xs text-muted-foreground/40">
                        Ativa em{" "}
                        {new Date(c.activated_at).toLocaleDateString("pt-BR", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "2-digit",
                        })}
                  ...
                  <div className="shrink-0 text-right text-xs text-muted-foreground/50">
                    <span className="tabular-nums">{c.analyses_count}</span>
                    <br />análises
                  </div>
                </button>
              ))}
